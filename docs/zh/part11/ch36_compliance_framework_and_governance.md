...
本章将从四个层面建立完整框架。第一，解释为什么合规是一种系统约束，而不是审批附件，并说明 Privacy by Design 的工程意义。第二，建立面向业务系统的数据分类分级、风险评估矩阵与责任链条，使团队能够明确“有哪些数据”“谁对这些数据负责”“什么场景下可以用、怎么用、谁批准”。第三，围绕处理活动记录（Record of Processing Activities，RoPA）、数据保护影响评估（Data Protection Impact Assessment，DPIA）、审计留痕和 CI/CD 阶段的自动化拦截机制，说明合规要求如何真正变成工程流程的一部分。第四，抽取一条最小治理链路，展示治理配置、脱敏策略、权限控制、预检机制、事件响应与复盘闭环如何落到具体系统产物中。
...
这一步关键，因为只有当制度语言进入系统元数据，合规才真正具备自动化执行的可能。
...
*代码清单36-6：审计日志 JSON 示例*

### 36.5.6 预检清单（JSON 示例）

代码清单36-7给出了上线前检查清单 JSON 示例。
...
为了说明治理模板不是纸面设计，表36-9以一条隐私治理流水线的常见产物为例，展示交付物如何映射到对应治理能力：
