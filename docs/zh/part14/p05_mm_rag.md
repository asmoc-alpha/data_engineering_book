...
当前核心粒度是**页面级视觉检索**。这适合处理图表和整页表格，但对于极长跨页表格、特别细碎的小区域问答，仍然可能需要进一步的 patch-level 或 region-level 检索增强。
...
ColPali 的价值在于，它不是把页面当普通自然图片来处理，而是更偏向**文档场景的视觉理解**。对财报、表格、图表、版面结构这类内容来说，这一点关键。
...
如果每次查询时才临时渲染 PDF，一方面会拖慢响应，另一方面会让调试变得较为麻烦。因为一旦某页渲染参数、缩放比例、裁剪逻辑变化，系统行为就可能前后不一致。
...
财报问答里，一个典型的问题是：**用户问的问题是跨页的，但系统只想返回一页**。
...
严格来说，这一步在很多原型演示中都被省略了。但在工程里，它需要重点关注。建议至少返回：
...
P05 当前指标较为整齐，这本身说明链路设计思路是对的。尤其是在复杂文档场景下，如果检索、引用和答案关键词都能保持一致，至少说明项目已经具备较好的闭环能力。
