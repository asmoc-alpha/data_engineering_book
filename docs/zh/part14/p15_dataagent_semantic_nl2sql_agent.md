...
如果按工程顺序阅读，本章对应的是一条从业务问题到可信交付的完整链路。
...
这一点关键。企业问数的交付物不应该只有一段自然语言回答，还应该包含可复核的 SQL、可下载的结果文件和后续报告依据。
