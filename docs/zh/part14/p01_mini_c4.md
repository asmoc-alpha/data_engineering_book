...
这一结构对应的核心目标，是在单机 CPU 与 Ray Data 条件下复现一条可解释、可复用的网页预训练数据流水线。
...
2. **重复度极高**：转载、镜像站、聚合页、模板页、页面局部复制较为普遍。
...
Mini-C4 的设计参考了 C4/T5 中“从网页语料构建可训练文本数据”的基本思路 (Raffel et al. 2020)，但本项目强调的是小规模工程复现，而不是复刻完整 C4 数据集。
...
Mini-C4 这个案例证明了一件事：  
即使在有限的边界下，项目依然可以把预训练数据工程中的关键问题讲清楚、做完整，并把它沉淀成能够复用的方法论。
...
Mini-C4 当前已经把英文和中文拆开处理，这一步关键，因为不同语言在网页结构、噪声类型和质量信号上差异很大。语言切分之后，质量打分就不应再只看统一阈值，而应结合语言特征判断。
...
P01 当前已经谈到前置域名过滤，这一点其实需要重点关注。因为预训练数据的很多成本，不是花在高价值内容上，而是花在海量低价值页面的下载、解析、清洗和去重上。未来如果要走向更真实的工厂形态，入口治理会变得越来越重要，包括：
