...
**原始图像资产 -> 衍生文档/图表资产 -> 指令合成 -> 区域对齐 -> 多图交错 -> 质量审核 -> 训练封装 -> 报告与验证**
...
代码清单P03-1 给出了流程或路径示例，用于说明本节中的输入输出关系、结构约束或执行方式。
```text
...
![图 P03-1：LLaVA 多模态指令数据工厂总览](../../images/part14/p03/Yu-Project03-Fig01.svg)
...
![图 P03-2：多模态数据工厂职责协同图](../../images/part14/p03/Yu-Project03-Fig02.svg)
...
![图 P03-3：多模态资产分层示意图](../../images/part14/p03/Yu-Project03-Fig03.svg)
*图 P03-3：多模态资产分层示意图*
...
![图 P03-4：文档图像任务分层图](../../images/part14/p03/Yu-Project03-Fig04.svg)
*图 P03-4：文档图像任务分层图*
...
代码清单P03-2 给出了 Python 实现片段，用于说明本节中的输入输出关系、结构约束或执行方式。
```python
# 核心代码摘自 alignment.py
...
![图 P03-5：bbox 坐标转换与归一化示意图](../../images/part14/p03/Yu-Project03-Fig05.svg)
...
代码清单P03-3 给出了 Python 实现片段，用于说明本节中的输入输出关系、结构约束或执行方式。
```python
import base64
...
代码清单P03-4 给出了 JSON 数据结构示例，用于说明本节中的输入输出关系、结构约束或执行方式。
```json
{
  "id": "p03_000128",
...
很多团队只保留“通过”的样本，而不系统记录“失败”的样本。这样做虽然省事，但会失去有价值的工程信号。
...
![图 P03-6：样本质检与回退闭环图](../../images/part14/p03/Yu-Project03-Fig06.svg)
*图 P03-6：样本质检与回退闭环图*
...
在本项目中，`visualize_bbox.py` 的意义典型。它证明了多模态数据工厂不能只在 JSON 层检查，而必须具备“反向渲染”能力。
...
其中 `smoke_test.jsonl` 关键。它不追求代表性，而追求能快速暴露字段缺失、图像路径错误、模板异常等问题。
...
![图 P03-7：失败样本归因示意图](../../images/part14/p03/Yu-Project03-Fig07.svg)
*图 P03-7：失败样本归因示意图*
...
![图 P03-8：项目验证闭环图](../../images/part14/p03/Yu-Project03-Fig08.svg)
*图 P03-8：项目验证闭环图*
...
LLaVA 类数据工厂还有一个需要重点关注、但经常被轻描淡写的部分，就是抽检与错误回放。因为多模态样本的错误往往并不体现在一句文本里，而是体现在图像、框、文本和任务关系的错位上。如果没有稳定的抽检和 replay 机制，团队很容易在样本量增加的同时失去对质量的直觉。
