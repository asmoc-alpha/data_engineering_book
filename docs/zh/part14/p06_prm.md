...
P06 的中间核心步骤是 `src/validate_and_score.py`，也就是把生成出来的轨迹做校验、打分与标签分配。这个步骤关键，因为它决定项目到底是在积累监督信号，还是在放大生成噪声。
...
现有指标显示，整体轨迹验证通过率为 `67.59%`，但正例轨迹通过率达到 `100.00%`，说明当前问题主要集中在 negative 与 repair 轨迹的控制上。这个结果有价值，因为它明确指出项目后续优化不应盲目扩量，而应优先提高清洗和验证质量。
...
现有指标显示，项目中存在 `144` 个 process-only supervision signal steps。这个数字的工程意义较为明确：过程监督确实提供了 outcome-only 无法替代的额外信号。如果只看最终结果，这 144 个步骤的价值会被直接忽略掉。
...
很多数据项目会把注意力放在 train/val 上，却忽略 smoke test 与 manifest。实际上，这两项需要重点关注。
...
这是一条需要重点关注的工程结论，因为它把“下一步做什么”从泛泛而谈，收敛成了明确的生产线问题。
...
现有报告明确指出，如果继续扩量而不先提高清洗和验证，PRM 数据会先变脏，而不是先变强。这个判断关键，因为它把工程优先级拉回到正确顺序：
...
P06 最值得保留的，并不是它已经做出了多大规模的 PRM 数据集，而是它清楚展示了一个关键的工程判断：
